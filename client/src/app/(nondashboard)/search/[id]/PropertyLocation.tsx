...
const PropertyLocation = ({ propertyId }: PropertyDetailsProps) => {
    ...
    useEffect(() => {
        ...
        const map = new mapboxgl.Map({
            ...
            style: 'mapbox://styles/tancaodev/cmarch1il01q701skc33a64hh',
            center: [property.location.coordinates.longitude, property.location.coordinates.latitude],
            zoom: 14
        })

        const marker = new mapboxgl.Marker().setLngLat([property.location.coordinates.longitude, property.location.coordinates.latitude]).addTo(map)
    ...
    return (
        <div className='py-16'>
            <h3 className='text-xl font-semibold text-primary-800 dark:text-primary-100'>Map and Location</h3>
            <div className='flex justify-between items-center text-sm text-primary-500 mt-2'>
                <div className='flex items-center text-gray-500'>
                    <MapPin className='w-4 h-4 mr-1 text-gray-700' />
                    Property Address:
                    <span className='ml-2 font-semibold text-gray-700'>{property.location?.address || 'Address not available'}</span>
                </div>
                <a
