    ...
    return (
        <div className='mb-6'>
                    ...
                    {property.amenities.map((amenity: AmenityEnum) => {
                        ...
                        return (
                            <div key={amenity} className='flex flex-col items-center border rounded-xl py-8 px-4'>
                                ...
                                <span className='text-sm text-center text-gray-700'>{formatEnumString(amenity)}</span>
                            </div>
                        )
                    })}
                </div>
            ...
            <div className='mt-12 mb-16'>
                <h3 className='text-xl font-semibold text-primary-800 dark:text-primary-100'>Highlights</h3>
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-4 w-full'>
                    {property.highlights.map((highlight: HighlightEnum) => {
                        ...
                        return (
                            <div key={highlight} className='flex flex-col items-center border rounded-xl py-8 px-4'>
                                <Icon className='w-8 h-8 mb-2 text-primary-600 dark:text-primary-300' />
                                <span className='text-sm text-center text-primary-600 dark:text-primary-300'>{formatEnumString(highlight)}</span>
                            </div>
                        )
                    })}
                </div>
            ...
            {/* Tabs Section */}
            <div>
                <h3 className='text-xl font-semibold text-primary-800 dark:text-primary-100 mb-5'>Fees and Policies</h3>
                ...
                <Tabs defaultValue='required-fees' className='mt-8'>
                    ...
                    <TabsContent value='parking'>
                        <p className='font-semibold mt-5 mb-2'>Parking is {property.isParkingIncluded ? 'included' : 'not included'}</p>
                    </TabsContent>
                </Tabs>
            </div>
        </div>
