...
export const CustomFormField: React.FC<FormFieldProps> = ({
    ...
    const renderFormControl = (field: ControllerRenderProps<FieldValues, string>) => {
        switch (type) {
            case 'textarea':
                return <Textarea placeholder={placeholder} {...field} rows={3} className={`border-gray-200 p-4 ${inputClassName}`} />
            case 'select':
                return (
                    <Select
            ...
            case 'number':
                return <Input type='number' placeholder={placeholder} {...field} className={`border-gray-200 p-4 ${inputClassName}`} disabled={disabled} />
            case 'multi-input':
                return <MultiInputField name={name} control={control} placeholder={placeholder} inputClassName={inputClassName} />
            default:
                return <Input type={type} placeholder={placeholder} {...field} className={`border-gray-200 p-4 ${inputClassName}`} disabled={disabled} />
        }
    }
                ...
                <FormItem className={`${type !== 'switch' && 'rounded-md'} relative ${className}`}>
                    {type !== 'switch' && (
                        <div className='flex justify-between items-center'>
                            ...
                            {!disabled && isIcon && type !== 'file' && type !== 'multi-input' && <Edit className='size-4 text-customgreys-dirtyGrey' />}
                        </div>
                    )}
                    <FormControl>
                        {renderFormControl({
        ...
        <div className='space-y-2'>
            {fields.map((field, index) => (
                <div key={field.id} className='flex items-center space-x-2'>
                    ...
                    />
                    <Button type='button' onClick={() => remove(index)} variant='ghost' size='icon' className='text-customgreys-dirtyGrey'>
                        <X className='w-4 h-4' />
                    </Button>
                </div>
            ))}
