...
const ApplicationCard = ({ application, userType, children }: ApplicationCardProps) => {
    const [imgSrc, setImgSrc] = useState(application.property.photoUrls?.[0] || '/placeholder.jpg')

    const statusColor = application.status === 'Approved' ? 'bg-green-500' : application.status === 'Denied' ? 'bg-red-500' : 'bg-yellow-500'
            ...
            <div className='flex flex-col lg:flex-row  items-start lg:items-center justify-between px-6 md:px-4 py-6 gap-6 lg:gap-4'>
                ...
                <div className='flex flex-col lg:flex-row gap-5 w-full lg:w-auto'>
                    ...
                    <div className='flex flex-col justify-between'>
                        <div>
                            <h2 className='text-xl font-bold my-2'>{application.property.name}</h2>
                        ...
                        <div className='text-xl font-semibold'>
                            ${application.property.pricePerMonth} <span className='text-sm font-normal'>/ month</span>
                        </div>
                    </div>
                </div>
                ...
                <div className='flex flex-col justify-between w-full lg:basis-2/12 lg:h-48 py-2 gap-3 lg:gap-0'>
                    <div>
                        <div className='flex justify-between items-center'>
                            ...
                            <span className={`px-2 py-1 ${statusColor} text-white rounded-full text-sm`}>{application.status}</span>
                        </div>
                        <hr className='mt-3' />
                    </div>
                    <div className='flex justify-between'>
                        <span className='text-gray-500'>Start Date:</span> {new Date(application.lease?.startDate).toLocaleDateString()}
                    </div>
                    <div className='flex justify-between'>
                        <span className='text-gray-500'>End Date:</span> {new Date(application.lease?.endDate).toLocaleDateString()}
                    </div>
                    <div className='flex justify-between'>
                        <span className='text-gray-500'>Next Payment:</span> {new Date(application.lease?.nextPaymentDate).toLocaleDateString()}
                    </div>
                </div>
                ...
                <div className='flex flex-col justify-start gap-5 w-full lg:basis-3/12 lg:h-48 py-2'>
                    <div>
                        <div className='text-lg font-semibold'>{userType === 'manager' ? 'Tenant' : 'Manager'}</div>
                        <hr className='mt-3' />
                    </div>
